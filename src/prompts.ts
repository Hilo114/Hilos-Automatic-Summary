...
/** 大总结系统提示词 — AI 判断模式（默认） */
export const DEFAULT_VOLUME_SUMMARY_AI_SYSTEM = `你是一个专业的故事总结助手。你的任务是将一系列小总结合并为一个完整的卷总结。
...
/** 大总结系统提示词 — 消息数触发模式（默认） */
export const DEFAULT_VOLUME_SUMMARY_COUNT_SYSTEM = `你是一个专业的故事总结助手。你的任务是将一系列小总结合并为一个完整的卷总结。
...
/** 获取当前生效的大总结系统提示词 — AI 判断模式 */
function getVolumeSummaryAiSystemPrompt(): string {
  const settings = getSettings();
  return settings.custom_prompts.volume_summary_system_ai || DEFAULT_VOLUME_SUMMARY_AI_SYSTEM;
}

/** 获取当前生效的大总结系统提示词 — 消息数触发模式 */
function getVolumeSummaryCountSystemPrompt(): string {
  const settings = getSettings();
  return settings.custom_prompts.volume_summary_system_count || DEFAULT_VOLUME_SUMMARY_COUNT_SYSTEM;
}
...
export function getVolumeSummaryPrompt(
  ...
  return {
    system: mode === 'ai' ? getVolumeSummaryAiSystemPrompt() : getVolumeSummaryCountSystemPrompt(),
    user: userPrompt,
  };
}
